...
# 4) Complete the SyncService implementation. Note that the SyncService.onMessage and SyncService.__init__ function
# signature must not be altered.
...
class SyncService:
    ...
    def onMessage(self, data: dict):
        ...
        Return the desired information in the correct format (type `update`, see Device.onMessage and
        testSyncing to understand format intricacies) in response to a `probe`.
        No return value required on handling a `record`."""

        # keys = [r for i,r in enumerate(data)]
        if not data:
            return{
                'type':'none'
            }
